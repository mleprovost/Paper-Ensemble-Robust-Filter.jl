export seqassim_lagrangian_likenrf
...
"""
		seqassim_lagrangian_likenrf(F::StateSpace, data::SyntheticData, J::Int64, ϵx::InflationType, algo::SeqFilter, X, Ny, Nx, t0::Float64)

Generic API for sequential data assimilation for any sequential filter of parent type `SeqFilter`.
	In this setting, the observations needs to be propa
"""
function seqassim_lagrangian_likenrf(F::StateSpace, data::SyntheticData, J::Int64, ϵx::InflationType, algo::LikEnRF, X, Ny, Nx, t0::Float64; isSDE::Bool = false)
	...
	if algo.estimate_dof == false 
		...
		# Estimate dof
			...
			if norm(mod(i, refresh_dof))<1e-12 && i*Ne >= Ne_buffer
				...
				νXestimated = EMq(buffer_joint, 0.97, 100; rtol = algo.rtol, 
