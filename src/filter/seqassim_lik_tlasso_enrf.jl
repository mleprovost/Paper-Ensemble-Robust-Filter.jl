export seqassim_likenrf_tlasso

# Write the seqassim routine for the LikEnRFLasso with further options to output the dof estimate over time and store previous joint samples
...
"""
function seqassim_likenrf_tlasso(F::StateSpace, data::SyntheticData, J::Int64, ϵx::InflationType, algo::LikEnRFLasso, X, Ny, Nx, t0::Float64; isSDE::Bool = false)
	...
	if algo.estimate_dof == false 
		...
		# Estimate dof
			...
			if norm(mod(i, refresh_dof))<1e-12 && i*Ne >= Ne_buffer
                ...
                νYXestimated  = regularized_EM_tdist_search(buffer_joint, algo.ρ_scaling[1]*1/sqrt(Ne), algo.νYXgrid; Niter = 500,
                                                                               tol = algo.rtol)[end]
                #@show νXestimated

				# print("New estimated νX value: $(νXestimated)")
				# Update dof of algo
				algo.νYX[1] = copy(νYXestimated)#
				#  LikEnRFLasso(algo.ϵy, algo.Δtdyn, algo.Δtobs, algo.Δtrefresh; rtol = algo.rtol,
							#    estimate_dof = algo.estimate_dof, νYX = [νXestimated], ρ_scaling = algo.ρ_scaling, νYXgrid = algo.νYXgridß)
			end
